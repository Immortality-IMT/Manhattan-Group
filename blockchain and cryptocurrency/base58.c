...
Test the base58 encoding function
    char in[] = "abcdefghijklmnopqrstuvwxyz1234567890";
    size_t in_len = strlen(in);
    char out[100];
    size_t out_len = sizeof(out);
    char decoded_out[100];
    size_t decoded_out_len = sizeof(decoded_out);

    memset(out, 0, sizeof(out));
    memset(decoded_out, 0, sizeof(decoded_out));

    // encode
    base58_encode(in, in_len, out, &out_len);
    printf("Input string: %s\nEncoded string: %s\n", in, out);

    // decode
    base58_decode(out, out_len, decoded_out, &decoded_out_len);
    decoded_out[decoded_out_len] = '\0';
    printf("Decoded string: %s\n", decoded_out);

    // verify that the original string and the decoded string are the same
    assert(strcmp(in, decoded_out) == 0);
*/
